    ...
    if denom == 0 or denom == 1:
        ...
        for i in xrange(len(my_set)-1):
            for j in xrange(i+1,len(my_set)):
                if my_set[i].shortest_path_distance(my_set[j]) != None:
                    counter = counter+my_set[i].shortest_path_distance(my_set[j])
...
killer_set = wn.synsets('save', pos=wn.VERB)
...
dog = wn.synset('lie.v.01')
cat = wn.synset('lie.n.01')
